...
with open('/home/ycshi/sticker-sentiment/ourdata/dess-yi/od.txt', 'r', encoding='utf-8') as f:
    datas = f.readlines()
for data in datas:
    if data.split('&&&')[0].endswith('.png'):
        dessi[data.split('&&&')[0]] = data.split('&&&')[1].replace('\n', '')
    else:
        dessi[data.split('&&&')[0].split('.')[0]+'.png'] = data.split('&&&')[1].replace('\n', '')
with open('/home/ycshi/sticker-sentiment/ourdata/dess-yi/ocr.txt', 'r', encoding='utf-8') as f:
    datas = f.readlines()
for data in datas:
    if data.split('&&&')[0].endswith('.png'):
        desso[data.split('&&&')[0]] = data.split('&&&')[1].replace('\n', '')
    else:
        desso[data.split('&&&')[0].split('.')[0]+'.png'] = data.split('&&&')[1].replace('\n', '')
with open('/home/ycshi/sticker-sentiment/ourdata/dess-yi/ti.txt', 'r', encoding='utf-8') as f:
    datas = f.readlines()
for data in datas:
    if data.split('&&&')[0].endswith('.png'):
        dessei[data.split('&&&')[0]] = data.split('&&&')[1].replace('\n', '')
    else:
        dessei[data.split('&&&')[0].split('.')[0]+'.png'] = data.split('&&&')[1].replace('\n', '')


dess = {}
for i in dessi.keys():
    dess[i] = dessi[i] + desso[i] + dessei[i]
...
def collate_fn(batch):
    ...
    for x in batch:
        ...
        sticker_text = dess[x['sticker']+ '.png']
        text = x['context'] + '。并发了张表情包，内容为：' + sticker_text
        # 将文本转化为token
        ...
        emo_lable.append(multimodal_intent_str2num[x['multimodal_intent_label']])
