...
with open('/home/ycshi/sticker-sentiment/shared_dataset/hard_task3/train.json', encoding='utf-8') as f:
    train = json.load(f)
with open('/home/ycshi/sticker-sentiment/shared_dataset/hard_task3/test.json', encoding='utf-8') as f:
    valid = json.load(f)
#with open('/home/ycshi/sticker-sentiment/shared_dataset/hard_task1/test.json', encoding='utf-8') as f:
    #test = json.load(f)
    ...
    for epoch in range(1,eopchs):
        ...
        with tqdm(total=len(train_loader), desc=f'Train Epoch {epoch}/{eopchs}', unit='batch', ascii=True) as pbar:
            for batch in train_loader:
                context_ids, context_masks, token_type_ids,sticker_txt_ids, stickertxt_masks,stickertxt_type_ids , emo_lable = batch
                ...
                loss, emo_classifier_output= model(context_ids, context_masks, token_type_ids,sticker_txt_ids ,stickertxt_masks,stickertxt_type_ids , emo_lable )
                ...
                pbar.update(1)
            print(f'Epoch {epoch}/{eopchs} train_loss: {train_loss/len(train_loader):.3f} emo_acc: {emo_acc/len(train_loader.dataset):.3f} ')
        ...
        with tqdm(total=len(valid_loader), desc=f'valid Epoch {epoch}/{eopchs}', unit='batch', ascii=True) as pbar:               
            with torch.no_grad():
                for batch in valid_loader:
                    context_ids, context_masks, token_type_ids,sticker_txt_ids,stickertext_masks,stickertext_type_ids , emo_lable = batch
                    context_ids = context_ids.to(device)
                    context_masks = context_masks.to(device)
                    token_type_ids = token_type_ids.to(device)
                    sticker_txt_ids = sticker_txt_ids.to(device)
                    ...
                    loss, emo_classifier_output= model(context_ids, context_masks, token_type_ids,sticker_txt_ids ,stickertext_masks,stickertext_type_ids , emo_lable)
                    emo_predict = torch.argmax(emo_classifier_output, dim=1)
                    #intent_predict = torch.argmax(intent_classifier_output, dim=1)
                ...
                print(f'Epoch {epoch}/{eopchs} valid_loss: {valid_loss/len(valid_loader):.3f} emo_acc: {val_emo_acc/len(valid_loader.dataset):.3f}')
                print(f'Epoch {epoch}/{eopchs} valid_loss: {valid_loss/len(valid_loader):.3f} emo_f1: {val_emo_f1/len(valid_loader):.3f}')
                if (val_emo_acc/len(valid_loader.dataset)) > max_emo[1]:
                    max_emo[0] = epoch
                    max_emo[1] = val_emo_acc/len(valid_loader.dataset)
                    max_emo[2] = val_emo_f1/len(valid_loader)
