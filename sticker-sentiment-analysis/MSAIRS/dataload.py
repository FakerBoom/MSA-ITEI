...
with open('/home/ycshi/sticker-sentiment/ourdata/dess-llava/odocrti.txt', 'r', encoding='utf-8') as f:
    datas = f.readlines()
for data in datas:
    if data.split('&&&')[0].endswith('.png'):
        dess[data.split('&&&')[0]] = data.split('&&&')[1].replace('\n', '')
    else:
        dess[data.split('&&&')[0].split('.')[0]+'.png'] = data.split('&&&')[1].replace('\n', '')
...
def collate_fn(batch):
    ...
    for x in batch:
        ...
        emo_lable.append(x['sticker_sentiment_label'])
